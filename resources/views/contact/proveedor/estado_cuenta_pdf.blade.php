...
<div class="titulo">ESTADO DE CUENTA POR PROVEEDOR</div>

<div class="subtitulo">
    <p>Proveedor: <strong>{{ $proveedor->name }} {{ $proveedor->supplier_business_name ?? '' }}</strong></p>
    <p>Documento: <strong>{{ $proveedor->contact_id }}</strong></p>
...
@foreach($comprasPorVenta as $transaction_id => $detalleCompra)
    ...
    @php
        ...
        $subtotalVenta = $detalleCompra->sum('importe_compra');
        $fechaVenta     = $detalleCompra->first()->fecha;
        $invoiceVenta = $detalleCompra->first()->ref_no;
    ...
    @for($i = 0; $i < $maxFilas; $i++)
        ...
        <tr>
            ...
            <td class="text-right">
                {{ isset($c) ? number_format($c->importe_compra,2) : '' }}
            </td>
    ...
    <tr class="totales">
        <td colspan="5" class="text-right">TOTAL COMPRA</td>
...
<tr>
    <td style="border:none; text-align:center"><strong>Firma del Proveedor</strong></td>
