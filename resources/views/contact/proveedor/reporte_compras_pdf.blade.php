...
<body>
    ...
    {{-- CABECERA --}}
    <div class="titulo">REPORTE DE COMPRAS POR PROVEEDOR</div>

    <div class="subtitulo">
        <strong>Proveedor:</strong> {{ $proveedor->name }} {{ $proveedor->supplier_business_name ?? '' }} <br>
    ...
    <table>
        ...
        @foreach($movimientos as $m)
            ...
            @if($m['es_primero'])
                <tr>
                    <td colspan="8" class="sin-borde">
                        <strong>Compra:</strong> {{ $m['factura'] }} &nbsp;&nbsp;
            ...
            @if($m['es_ultimo'])
                <tr class="subtotal">
                    <td colspan="7" class="text-right">SUBTOTAL COMPRA</td>
                    <td class="text-right">
